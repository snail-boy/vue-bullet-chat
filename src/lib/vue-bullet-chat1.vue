<template>
  <div class="vue-bullet-chat-wrapper" :style="background" @click="bulletChatClick">
    ...
    <div class="vbc-text-wrapper">
      <div class="vbc-text" :style="textObj">{{value}}</div>
    </div>
    <div class="vbc-input-wrapper vbc-flex-sb vbc-input-wrapper-active" @click.stop>
      <input class="input-box" v-model="setValue" placeholder="请输入弹幕文字" @focus="handleFocus" @input="handleInput" />
      <div class="svg-box">
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.textWrapper.style.top = window.innerHeight + 'px'
        this.lockClose = this.utils.classEle('vbc-icon-close')
        this.lockOpen = this.utils.classEle('vbc-icon-open')



        this.innerHeight = Math.floor((this.textWrapper.getBoundingClientRect().width - window.innerWidth) / 2)
        this.textWrapper.style.left = - this.innerHeight + 'px'
        this.textWrapper.style.top = '50%'
        this.textWrapper.style.marginTop = -Math.floor(this.textWrapper.getBoundingClientRect().height / 2) + 'px'
        this.textWrapper.style.transform = 'rotate(90deg)'
        this.initHeight = Math.floor((window.innerHeight - (window.innerHeight - this.textWrapper.getBoundingClientRect().height) / 2) + window.innerHeight / 2)
        this.initHeightCopy = JSON.parse(JSON.stringify(this.initHeight))

        console.log(this.initHeight , 'this.initHeight ')
        this.textWrapper.style.top = this.initHeight + 'px'
        ...
        this.timer = setInterval(() => {
          this.i += this.speed
          value = this.initHeightCopy - this.i
          this.textWrapper.style.top = value + 'px'
          if (this.textWrapper.getBoundingClientRect().top <= -this.textWrapper.getBoundingClientRect().height) {
            value = 0
            this.i = 0
            this.textWrapper.style.top = this.initHeight + 'px'
          }
